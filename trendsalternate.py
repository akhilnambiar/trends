...
def calculate_average_sentiments(tweets_by_state):
    ...
    ana_sum=0
    index=0
    averaged_state_sentiments = {x:0 for x in tweets_by_state.keys()}
    for i in tweets_by_state.keys():
        ana=0
        ana_sum=0
        index=0
        for j in tweets_by_state[i]:
            ana=analyze_tweet_sentiment(j)
            if ana!=None:
                ana_sum+=ana
                index+=1
        ana_sum=ana_sum/index
        averaged_state_sentiments[i]=ana_sum
    for i in tweets_by_state.keys():
        if averaged_state_sentiments[i]==0 or averaged_state_sentiments[i]==None:
            del averaged_state_sentiments[i]
